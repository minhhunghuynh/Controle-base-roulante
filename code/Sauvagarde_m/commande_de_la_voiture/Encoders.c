...
void acceleration(float acc,int16_t timer,int16_t* etape, int dist_acc){
    ...
    while (ref_pwm < 0.5 & ref_pwm < 0.5){
        ...
        //if(_T1IF){// partie retroaction (beaucoup plus fr�quente)
            //_T1IF = 0;
        }
      ...
      // test pour les petites distances ; condition sera ok si les distances sont tr�s petites
      //int pos1 = enc_getPos1() ; int pos2 = enc_getPos2() ; 
