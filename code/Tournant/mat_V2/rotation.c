...
// Configures the quadrature encoders: FIT0450
void enc_config(void) {
    // voir page 6 du lien suivant: https://ww1.microchip.com/downloads/en/DeviceDoc/70208C.pdf
    ...
    QEI1CONbits.SWPAB = 1;      // swap between CHA and CHB
    ...
    QEI2CONbits.SWPAB = 0;      // no swap pour compte dans sens positif; les deux canaux de l'encodeur échangés
...
void setup_PWM(void){
    ...
    OC1R = 0; // initialiser OCR1
    OC1RS = 1; // Min 1 max PR2, va réactualiser OC1R à la fin du timer associé
...
void v_avant(uint16_t seuil){ // faire reset(voir en haut))
    QEI1CONbits.SWPAB = 1;
    QEI2CONbits.SWPAB = 1;  // vérifier que cela compte bien en positif 
    _LATB8 = 0 ; // roue 1
    _LATB11 = 1 ; //roue 2
    uint16_t pos1 = 0;
    uint16_t pos2 = 0;
while (pos1 < seuil && pos2<seuil){// change type && POS2CNT > seuil
    OC1RS = PR2*0.4 ; //roue gauche(avec bille avant))
    OC2RS = PR2*0.4; // roue droite
    pos1 = enc_getPos1();
    pos2 = enc_getPos2();
...
int16_t mettre_to_pos(float cm){
    ...
    void acceleration(float but1,float but2,int16_t acc,int16_t timer){
    int pwm1 = 1;
    int pwm2 = 1;
    PR3 = timer; 
    T3CONbits.TON = 1; // démarrer timer
    while (pwm1 < but1 && pwm2 < but2){
        if(_T3IF){
            _T3IF = 0;
            pwm2 = pwm2 + acc;
            pwm1 = pwm1 + acc;
