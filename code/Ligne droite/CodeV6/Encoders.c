...
void correction(int16_t pos1, int16_t pos2, float ref1, float ref2, float coef){
    ...
    if (pos1 > pos2){
        err = pos1 - pos2;
        if (OC1RS > 0.4*PR2){ // on le restricte pour ne pas avoir d'oscillation extrème entre etteint et allumé
            OC2RS = ref2*PR2 + coef*err;
            OC1RS = ref1*PR2 - coef*err;}
    }
    if (pos2 > pos1){
        err = pos2 - pos1;
        if (OC1RS < PR2*0.6){ // on le restricte pr pas explosion
            OC2RS = ref2*PR2 - coef*err;
            OC1RS = ref1*PR2 + coef*err; }
    }
