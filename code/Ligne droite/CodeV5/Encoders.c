...
void enc_config(void) {
    ...
    _QEB2R = 7;               // CHBA assignment to its RP6 pin
...
void correction(int16_t pos1, int16_t pos2){
    // Peut etre que la pos est pas bien prise
    float coef = 0.006; 
    if (pos1 > pos2){
        if (OC1RS > 0.57*PR2){ // on le restricte pour ne pas avoir d'oscillation extr�me entre etteint et allum�
            //_LATB2 = 1;
        OC2RS = OC2RS + (PR2*coef);
        OC1RS = OC1RS - (coef*PR2);}
        
    }
    if (pos2 > pos1){
        if (OC1RS < PR2*0.63){ // on le restricte pr pas explosion
            //_LATB4 = 0;
        OC1RS = OC1RS + (coef*PR2);}
        OC2RS = OC2RS - (PR2*coef); 
...
int16_t mettre_to_pos(int16_t cm){
    long inter = cm*67.9;
    int16_t p = (int16_t)inter; // ne donne pas le bon r�sultat ??
    return p;
