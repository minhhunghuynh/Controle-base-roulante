...
Code divisé en 4 parties pour lreception du essage et transfo en bit :
    - Etape 0 : Attente du start bit (lis sur l'ADC et corrélation)
    - Etape 1 : Effectue les mesures du reste de la trame sur l'ADC
    - Etape 2 : Convertit ces mesure vers bit 0 ou 1 et interprete les données pour renvoyer vers etape 3,4 ou 5
    - Etape 3 : Avance en ligne droite
    - Etape 4 : Tourne dans le sens anti-horlogique
    - Etape 5 : Torne dans le sens horlogique
*/
...
int main(void) {;
    ...
    int16_t dist_r1 = (int16_t) 1.01*dist_pos; // Pour que la roue 1 fasse un peu plus de chemin
    int16_t dist_r2 = dist_pos;
    
    
    //Variable pour les pos de roues !!! les position sont maintenant prise ne relatives par enc_getpos
    int16_t x = 0; 
    int16_t y = 0; 
    float err = 0;
    ...
    while(1==1) {
        ...
        if (etape ==0){ // Recherche le start bit
            // Après deux détection de 800Hz successive passe à l'etape 1
            ech_signal(n_ech, t_ech , &liste);
            r = correl(liste, n_ech,&temps,rif);
            ...
            if (r == 0){
                ...
                if (cnt == 2){ // On veut deux détections de suite
                    ...
                    ech_signal(n_ech, t_ech , &liste);
                    i = 1; // reset i
                    cnt = 0;}} // Reset cnt pour prochaine startbit
            else{cnt =0;}
        }


        if (etape == 1){ // Echantillonne tout les bit puis passe à l'étape 2
        ...
        if (etape == 2){ // Fait la correlation sur chaque bit, vérifie la parité et interprete les données
            n_ech = n_ech *2; 
            for (int i = 0; i < 11; i++){
            ...
            if (v == 0){ // verif de parité
                if (ordre == 1) { // Partie ligne droite
                    ...
                    etape_ligne = 1; // Si = 1 => fait l'accéleration
                    dist_cm = octet_to_int(lst);
                    dist_pos = mettre_to_pos((float)dist_cm);
                    dist_r2 = (int16_t) dist_pos;
                    float ajout_dist = 1.004*dist_pos;
                    dist_r1 = (int16_t)  ajout_dist;
                    dist_decel = 9000 ;
                    dist_pos = 10000; // distance a parcourrir
                    if (sens == 1){
                        _LATB8 = 1;
                        _LATB11 = 1;
                        QEI1CONbits.SWPAB = 1;
                ...
                if (ordre == 0) {  // Tourne 
                    ...
                    seuil = 10.15* nb_degre; // Coefficient expérimental
                    if (sens == 0){etape = 4;}
                    if (sens == 1){etape = 5;}
        ...
        if (etape == 3){ // Ligne droite
            _LATB3 = 0;
            if (etape_ligne == 1){
                acceleration(acc,timer,p_etape_ligne,dist_pos,dist_r1,dist_r2);
        }
            if (etape_ligne == 2){
                if(dist_decel< enc_getPos1(dist_r1) && dist_decel< enc_getPos2(dist_r2)){
                    etape_ligne = 3;}
            ...
            if (*p_etape_ligne == 3){
                deceleration(0.005, 0.5, 20, dist_pos, 0.25, p_etape_ligne,dist_r1,dist_r2);
                enc_reset();
                etape = 0;
                // Parfois fait un tournant apres ligne droite attente de 40ms
                i = 0;
                T1CONbits.TON = 1;
                while(i<4){while(_T1IF == 0){} _T1IF = 0;i = i +1;}
                T1CONbits.TON = 0;
                i = 0;
        }}
