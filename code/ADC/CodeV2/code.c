...
int main(void) {
    ...
    // Temps :
    float t_ech = 3.3; // En milisencode
    float f_ech = 10000;
    ...
    int16_t rep = 0;
    signed int liste[(signed int)n_ech]; 
    int16_t res[12];
    ...
    // On veut 1.4 ms
    PR1= 1.4*0.001*FCY / 8; // 1.4*0.001*FCY /8=  
...
T1CONbits.TON = 1;
    ...
    while(1) {
        ...
        if (etape ==0){
            ...
            //int r1 = TMR1 ;
            int r = correl(liste, (int)n_ech,&temps); //fct
             int16_t y =(int16_t) (r);
        ...
        if (etape == 1){
            ...
            if (_T1IF){
                ...
                if (i == 1){ech_signal(n_ech, t_ech, &L1);
                    // On veut 6.7 ms
                    T1CONbits.TCKPS = 2; // set le prescaler a 1:64
                    PR1= 0.067*FCY /64;}
        ...
        if (etape == 2){
            ...
            for (int i = 0; i < 11; i++){
                ...
                if (i == 0){lst[i] = correl(L1, n_ech,&temps);}
                if (i == 1){lst[i] = correl(L2, n_ech,&temps);}
                if (i == 2){lst[i] = correl(L3, n_ech,&temps);}
                if (i == 3){lst[i] = correl(L4, n_ech,&temps);}
                if (i == 4){lst[i] = correl(L5, n_ech,&temps);}
                if (i == 5){lst[i] = correl(L6, n_ech,&temps);}
                if (i == 6){lst[i] = correl(L7, n_ech,&temps);}
                if (i == 7){lst[i] = correl(L8, n_ech,&temps);}
                if (i == 8){lst[i] = correl(L9, n_ech,&temps);}
                if (i == 9){lst[i] = correl(L10, n_ech,&temps);}
                if (i == 10){lst[i] = correl(L11, n_ech,&temps);}
