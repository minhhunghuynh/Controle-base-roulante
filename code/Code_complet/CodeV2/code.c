...
int main(void) {
    ...
    PR1= (0.01)*FCY /8; // Chaques echantillonnage du message se fait a un intervale de 10ms
    // Vecteur temps
...
//  int16_t  send= 0 ;
    ...
    while(1==1) {
        ...
        if (etape == 1){
            ...
            if (_T1IF == 1){
               ...
               _LATB2 = 0;
                ...
                if (i == 1){ech_signal(2*n_ech, 2*t_ech, &L1);
                _T1IF = 0;
                while (_T1IF == 0){}}
        ...
        if (etape == 2){
            ...
            for (int i = 0; i < 11; i++){
                if (i == 0){ordre = correl(L1, 2*n_ech,&temps,0);sum = sum + ordre;}
                if (i == 1){sens = correl(L2, 2*n_ech,&temps,0);sum = sum + sens;}
                if (i == 2){lst[0] = correl(L3, 2*n_ech,&temps,0);sum = sum + lst[0];}
                if (i == 3){lst[1] = correl(L4, 2*n_ech,&temps,0);sum = sum + lst[1];}
                if (i == 4){lst[2] = correl(L5, 2*n_ech,&temps,0);sum = sum + lst[2];}
                if (i == 5){lst[3] = correl(L6, 2*n_ech,&temps,0);sum = sum + lst[3];}
                if (i == 6){lst[4] = correl(L7, 2*n_ech,&temps,0);sum = sum + lst[4];}
                if (i == 7){lst[5] = correl(L8, 2*n_ech,&temps,0);sum = sum + lst[5];}
                if (i == 8){lst[6] = correl(L9, 2*n_ech,&temps,0);sum = sum + lst[6];}
                if (i == 9){lst[7] = correl(L10, 2*n_ech,&temps,0);sum = sum + lst[7];}
                if (i == 10){parite = correl(L11, 2*n_ech,&temps,0);sum = sum + parite;}   
    ...
    // Envoye l'info via UART en boucle
        ...
        if (i>8){i = 0;}
            ...
            if (v == 0){ // Pas encore de verif (if v)
